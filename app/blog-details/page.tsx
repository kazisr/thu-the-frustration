...
const BlogDetailsPage = () => {
  ...
  useEffect(() => {
    const storedBlogString = localStorage.getItem("selectedBlog");
    if (storedBlogString) {
      try {
        const parsedBlog: Blog = JSON.parse(storedBlogString);
        setBlog(parsedBlog);
      } catch (error) {
        console.error("Error parsing blog from localStorage", error);
      }
    }
  }, []);
              ...
              <div>
                ...
                <div>
                  <div className="relative z-10 mb-10 overflow-hidden rounded-md bg-primary bg-opacity-10 p-8 md:p-9 lg:p-8 xl:p-9">
                    <p className="justify-between text-justify text-base font-medium italic text-body-color">
                      {blog?.paragraph}
                    </p>
